...
import styles from '../../comercial/pipeline/pipeline.module.css'
import { Suspense } from 'react'
import LeadDetailPanel from '../../comercial/pipeline/LeadDetailPanel'
import { CnpjLookupCard, CnpjImportField, formatCnpj } from '../../comercial/pipeline/CnpjLookup'
import {
...
function PipelineContent() {
  ...
  const [headerClienteNome, setHeaderClienteNome] = useState('')
  ...
  useEffect(() => {
    ...
    if (s.role !== 'ngp' && s.role !== 'admin' && s.role !== 'cliente') { router.replace('/cliente'); return }
    if ((s.role === 'admin' || s.role === 'ngp') && !clienteId) {
      router.replace('/comercial-digital')
      return
    }
    setSess(s)
  }, [router, clienteId])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled && (!res.ok || data.error || !data.access?.crm_enabled)) {
        ...
        if (!cancelled) router.replace('/cliente')
      }
    })()
  ...
  useEffect(() => {
    if (sess?.role === 'cliente') {
      setHeaderClienteNome(sess.user || 'Cliente')
      return
    }
    if (clienteNomeParam?.trim()) {
      setHeaderClienteNome(clienteNomeParam.trim())
      return
    }
    setHeaderClienteNome(clienteId ? 'Cliente selecionado' : '')
  }, [sess, clienteId, clienteNomeParam])

  // ── Load pipelines ────────────────────────────────────────────────────────
  const loadPipelines = useCallback(async () => {
    const data = await crmRequest('crm-manage-pipeline', { action: 'list' })
  ...
  const loadInitialData = useCallback(async (pipelineId?: string) => {
    ...
    setError('')
    try {
      const data = await crmRequest('crm-manage-pipeline', {
      ...
      if (data.error) {
        ...
        if (data.error.includes('desconhecida')) {
          const pls = await crmRequest('crm-manage-pipeline', { action: 'list' })
          if (pls.pipelines) {
            ...
            if (firstId) {
              const [sData, lData, fData, tData] = await Promise.all([
                crmRequest('crm-manage-stages', { action: 'list', pipeline_id: firstId }),
                crmRequest('crm-manage-leads',  { action: 'list', pipeline_id: firstId }),
                crmRequest('crm-manage-fields', { action: 'list', pipeline_id: firstId }),
                crmRequest('crm-manage-tasks',  { action: 'list_team', status: 'pendente' })
              ])
  ...
  async function createPipeline() {
    ...
    setSaving(true)
    const data = await crmRequest('crm-manage-pipeline', { action: 'create', name: fPipelineName, description: fPipelineDesc })
    setSaving(false)
  ...
  async function deletePipeline() {
    ...
    setSaving(true)
    const data = await crmRequest('crm-manage-pipeline', { action: 'delete', pipeline_id: activePipelineId })
    setSaving(false)
  ...
  async function createLead() {
    ...
    try {
      const data = await crmRequest('crm-manage-leads', {
  ...
  async function deleteLead() {
    ...
    const data = await crmRequest('crm-manage-leads', { action: 'delete', lead_id: leadToDelete.id })
    if (data.error) {
      showErr(data.error)
  ...
  async function saveStages() {
    ...
    try {
      ...
      if (newStageName.trim()) {
        const data = await crmRequest('crm-manage-stages', { action: 'create', pipeline_id: activePipelineId, name: newStageName })
        if (data.error) { showErr(data.error); return }
      }
      ...
      for (const se of stageEdits) {
        ...
        if (orig && (orig.name !== se.name || orig.color !== se.color)) {
          if (orig.name !== se.name)   await crmRequest('crm-manage-stages', { action: 'rename',       stage_id: se.id, name: se.name })
          if (orig.color !== se.color) await crmRequest('crm-manage-stages', { action: 'update_color', stage_id: se.id, color: se.color })
        }
      }
      // Reordenar
      await crmRequest('crm-manage-stages', { action: 'reorder', pipeline_id: activePipelineId, ordered_ids: stageEdits.map(s => s.id) })
    } finally {
      setSaving(false)
    }
  ...
  async function deleteStage(stageId: string) {
    ...
    const data = await crmRequest('crm-manage-stages', { action: 'delete', stage_id: stageId })
    if (data.error) { showErr(data.error); return }
  ...
  return (
    <div className={styles.layout}>
      <Sidebar 
        minimal 
        sectorNav={sess?.role === 'cliente' ? clientPortalNav : comercialDigitalNav} 
        sectorNavTitle={sess?.role === 'cliente' ? 'ÁREA DO CLIENTE' : 'COMERCIAL DIGITAL'} 
        activeTab={viewMode}
        onTabChange={(tab) => {
          if (sess?.role === 'cliente') {
        ...
        <div className={styles.content}>
          ...
          <header className={styles.header}>
            <div className={styles.headerLeft}>
              <div>
                <span className={styles.eyebrow}>SETOR COMERCIAL DIGITAL</span>
                <h1 className={styles.title}>
                  {viewMode === 'kanban'
                    ? (clienteId || sess?.role === 'cliente' ? 'Meu CRM Digital' : 'CRM do Cliente')
                    : viewMode === 'funil'
                      ? 'Funil Digital'
                      : viewMode === 'fields'
                        ? 'Campos do CRM'
                ...
                {headerClienteNome && (
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 10, padding: '8px 12px', borderRadius: 999, background: '#fff1f5', color: '#9b153f', fontSize: 13, fontWeight: 800 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#9b153f', display: 'inline-block' }} />
                    Cliente: {headerClienteNome}
                  </div>
                )}
              </div>
            ...
            <div className={styles.headerRight}>
              {sess?.role === 'cliente' && (
                <button className={`${styles.btn} ${styles.btnGhost}`} onClick={() => router.push('/cliente')}>
                  ← Ferramentas
                </button>
              )}
      ...
      {showEditLead && editLead && (
        <LeadDetailPanel
          ...
          onClose={() => {
            ...
            if (closedLeadId) {
              crmRequest('crm-manage-tasks', { action: 'list', lead_id: closedLeadId }).then(res => {
